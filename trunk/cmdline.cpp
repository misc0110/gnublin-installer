...
int main(int argc, char* argv[]) {
  ...
  if(action == "install" || action == "i") {
    ...
    if(verbose) {
     ...
     if(!bootl.empty()) cout << "Bootloader: \t" << bootl << endl;
     if(!kernel.empty()) cout << "Kernel: \t" << kernel << endl;
     if(!rootfs.empty()) cout << "RootFS: \t" << rootfs << endl;
    ...
    if(is_mounted(part.c_str())) {
      if(verbose) cout << "Unmounting '" << part << "'" << endl;
    ...
    // repartition
    if(repart) {
     if(verbose) cout << "Creating partitions (bootloader partition: " << bsize << "MB)" << endl;
    ...
    if(!bootl.empty()) {
     ...
     if(verbose) cout << "Writing bootloader to partition '" << boot_part << "'" << endl;
     dd(bootl.c_str(), boot_part.c_str(), 512);
    }
    
    // mounting partition
    if(verbose) cout << "Mounting partition" << endl;
    mkdir("linux", 0777);
    ...
    // extract rootfs
    if(!rootfs.empty()) {
     if(verbose) cout << "Extracting RootFS" << endl;
     extract_archive(rootfs.c_str(), "linux"); 
    }
    
    // copy kernel
    if(!kernel.empty()) {
     if(verbose) cout << "Copying kernel" << endl;
     copy_file(kernel.c_str(), "linux/zImage"); 
    }
    
    // unmounting
    if(verbose) cout << "Unmounting partition" << endl;
    unmount_partition("linux");
    
    //syncing
    if(verbose) cout << "Syncing card" << endl;
    sync_card();
    ...
    if(verbose) cout << dev_size << " kb to copy" << endl;
    
    if(verbose) cout << "Creating backup" << endl;
    ...
    if(verbose) cout << "Restoring backup" << endl;
    copy_device(in, out, dev_size, verbose);
...
void copy_device(FILE* in, FILE* out, long long kbytes, bool verbose) {
  ...
  while(!feof(in)) {
    ...
    if(percent != 0) {
      ...
      if(verbose) {
	if((int)(percent / 2) != last_ten) {last_ten = (int)(percent / 2); cout << "." << flush;}
      }
    }
  }
